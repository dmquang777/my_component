...
class FRadio<T> extends StatefulWidget {
  const FRadio({
    ...
    this.enable = true,
    this.uncheckBorderColor = const Color(0xffE5EAF0),
    this.inactiveBorderColor = const Color(0xffCFD7E3),
...
class _FRadioState extends State<FRadio> {
  ...
  Widget build(BuildContext context) {
    final double widthCircle = widget.size.outterRadius <= 48 ? 48 : widget.size.outterRadius * 2;
    ...
    return AbsorbPointer(
      ...
      child: InkWell(
        ...
        borderRadius: BorderRadius.all(Radius.circular(widthCircle)),
        child: SizedBox(
          width: widthCircle,
          height: widthCircle,
          child: Align(
            ...
            child: Container(
              width: widget.size.outterRadius * 2,
              height: widget.size.outterRadius * 2,
              decoration: BoxDecoration(
                color: widget.enable ? null : (widget.value == widget.groupValue ? null : widget.inactiveColor),
                borderRadius: BorderRadius.all(Radius.circular(widget.size.outterRadius)),
              ),
              child: _Radio(
                ...
                activeColor: widget.enable ? widget.activeColor : widget.inactiveBorderColor,
                splashRadius: widget.splashRadius,
                size: widget.size,
                inactiveBorderColor: widget.inactiveBorderColor,
                inactiveColor: widget.inactiveColor,
                uncheckBorderColor: widget.uncheckBorderColor,
...
/// be selected. The values are of type `T`, the type parameter of the [_Radio]
...
/// maintained by the example's `State`. In this case, the first [_Radio]
...
class _Radio<T> extends StatefulWidget {
  ...
  /// of the [_Radio], providing a different [Color] when it is
  /// [MaterialState.disabled].
  ///
  /// ```dart
  /// Radio<int>(
...
class _RadioState<T> extends State<_Radio<T>> with TickerProviderStateMixin, ToggleableStateMixin {
  ...
  @override
  void didUpdateWidget(_Radio<T> oldWidget) {
    ...
    return Semantics(
      ...
      child: buildToggleable(
        ...
        painter: _painter
          ...
          ..inactiveBorderColors = widget.inactiveBorderColor
          ..enable = widget.enable
          ..uncheckBorderColor = widget.uncheckBorderColor,
      ),
    );
...
class _RadioPainter extends ToggleablePainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Paint paint = Paint()
      ..color = enable ? Color.lerp(uncheckBorderColor, activeColor, position.value)! : inactiveBorderColors!
    ...
    if (!position.isDismissed) {
      ...
      canvas.drawCircle(center, (sizeValue!.outterRadius / 2) * position.value, paint);
    }
  }
}
